"""ENA Portal REST client for fetching Woolly Mammoth run records and FASTA files."""
...
class ENAClientError(RuntimeError):
    """Raised when an ENA Portal API request fails."""

    def __init__(self, project_accession: str, status_code: int):
        super().__init__(
            f"ENA request failed for project {project_accession}: HTTP {status_code}"
        )
...
class ENAClient:
    """Wraps the ENA Portal API for the Mammoth project PRJEB59491."""
    ...
    def fetch_project_runs(self, project_accession: str) -> list[dict[str, Any]]:
        """Return list of run records or raise ENAClientError."""
        url = (
            f"{self.base_url}/filereports"
            f"?accession={project_accession}&result=read_run&fields=run_accession"
        )
        try:
