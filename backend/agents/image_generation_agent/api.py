"""HTTP boundary for the Image Generation Agent.
...
Run it (from the repository root, with this agent's venv active):
    ...
    python -m uvicorn backend.agents.image_generation_agent.api:app --port 8009
"""
...
def execute(request: AgentRequest) -> AgentResult:
    ...
    try:
        ...
        # that contract.
        return AgentResult(
            status=AgentStatus.FAILED, output=f"Image Generation Agent error: {exc}"
        )
