...
async def _run_case_async(case: dict, verbose: bool = False) -> EvalResult:
...
def run_case(case: dict, verbose: bool = False) -> EvalResult:
    """
    ...
    verbose: print each graph node live as it fires, instead of only the
    one-line summary once the case finishes.
    """
...
def main(
    ...
    verbose: stream each case's graph execution live (node fired + tool
    calls made, with elapsed time) instead of only printing the one-line
    summary after each case completes.
    """
    cases = load_cases()
    if filter_id:
    ...
    for case in cases:
        print(f"  [{case['id']}] {case.get('category', '')}", flush=True)
        if not verbose:
            print("    ", end="", flush=True)
        actual = run_case(case, verbose=verbose)
        all_results.append((case, actual))
        prefix = "    -> " if verbose else ""
        print(
            f"{prefix}status={actual.status}  "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Stream each case's graph execution live — node fired + tool "
             "calls made, with elapsed time — instead of only a one-line "
             "summary once the case finishes. Also raises the log level "
    ...
    main(filter_id=args.case, filter_cat=args.category, verbose=args.verbose)
