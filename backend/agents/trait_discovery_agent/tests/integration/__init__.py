"""Deterministic stand-ins for the external boundaries the integration tests must
not touch: QuickGO (GO), KEGG, UniProt, and the NVIDIA NIM LLM.

Everything else in these tests runs for real: the compiled orchestrator graph, the
compiled sub-orchestrator graph, the subagent business logic (dedup, status rules),
and the Qdrant caching / retrieval code in kb/.
"""
