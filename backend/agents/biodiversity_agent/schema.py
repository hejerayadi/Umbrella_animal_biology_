...
class AgentStatus(Enum):
    ...
    The first four are the platform-wide contract every agent shares. The last
    two are the extra outcomes the M2 and M3 design documents require
    (``Biodiversity Hotspots.pdf`` Table 14, ``Habitat Visualization.pdf``
    Table 13) and are used *inside* this agent:

    * ``PARTIAL`` - real work was produced but a requested piece failed. Doc
      §6.3 rule 5: partial success is a first-class outcome, not a failure.
    * ``NEEDS_CLARIFICATION`` - the region or species could not be resolved, so
      the agent asks rather than inventing an answer.

    Both are narrowed to the four platform statuses at the HTTP boundary in
    ``api.py``, which is the only place that talks to the Global Orchestrator.
    """
