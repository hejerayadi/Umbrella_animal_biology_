...
class BiodiversityOrchestrator:
    ...
    def __init__(
        ...
        self._workers = workers or self._default_workers(use_real_models)
    ...
    @staticmethod
    def _default_workers(use_real_models: bool | None) -> dict[BiodiversityFeature, Any]:
        """Pick the mock or the real worker for each feature.

        Mocks stay the default so the test suite runs offline and instantly, which
        is what keeps Sprint 2's orchestration tests honest. The real M3 worker
        reaches GBIF and takes tens of seconds, so it is opted into - by
        ``use_real_models=True``, or by setting ``BIODIVERSITY_REAL_MODELS=1`` in
        the environment. ``dashboard_modules.py`` calls it directly.

        M1, M2 and M4 have no real pipeline yet, so they stay on their fixtures
        either way.
        """

        if use_real_models is None:
            use_real_models = os.environ.get("BIODIVERSITY_REAL_MODELS") == "1"

        workers: dict[BiodiversityFeature, Any] = {
        ...
        if use_real_models:
            # Imported lazily: it pulls in scikit-learn, which the mock-only path
            # has no reason to load. This line is the whole of Sprint 3 task 4 for
            # M3 - the worker presents the same run(request) -> AgentResult
            # contract as the mock it replaces.
            from ..workers.hotspots.worker import HotspotsWorker

            workers[BiodiversityFeature.BIODIVERSITY_HOTSPOTS] = HotspotsWorker()
