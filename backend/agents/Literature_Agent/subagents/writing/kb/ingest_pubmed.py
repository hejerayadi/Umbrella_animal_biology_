"""
Ingestion du dataset PubMed (ccdv/pubmed-summarization) dans la collection
Qdrant ghaya_papers_fulltext, filtree strictement sur la biodiversite
ANIMALE (exclut plantes/algues/microbes). Filtre applique sur l'abstract
uniquement (signal plus fiable que le debut du corps).

Usage (depuis backend/, avec le venv active) :
    python -m agents.Literature_Agent.subagents.writing.kb.ingest_pubmed
"""
...
def build_points_for_article(article: dict, article_idx: int) -> tuple[list[str], list[dict], list[str]]:
    ...
    if abstract.strip():
        for chunk_idx, chunk in enumerate(chunk_section_text(abstract)):
            ...
            ids.append(f"pubmed_{article_id}_abstract_{chunk_idx}")
    ...
    if body.strip():
        for chunk_idx, chunk in enumerate(chunk_section_text(body)):
            ...
            ids.append(f"pubmed_{article_id}_body_{chunk_idx}")

    return texts, payloads, ids


def ingest_pubmed(target_matches: int = 100, max_scan: int = 20000):
    print("Chargement du dataset PubMed (ccdv/pubmed-summarization)...", flush=True)
    ...
    print("Ingestion PubMed terminee.", flush=True)


if __name__ == "__main__":
    ingest_pubmed(target_matches=100, max_scan=20000)