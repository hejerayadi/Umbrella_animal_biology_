...
/**
 * The map URL an agent returned, if any. Looked for at the top level and one
 * level down, because the orchestrator nests each agent's payload under its own
 * key when several ran.
 */
function readMapUrl(context: Record<string, unknown> | undefined): string | undefined {
  if (!context) return undefined;
  const direct = context.map_url;
  if (typeof direct === "string" && direct.length > 0) return direct;
  for (const value of Object.values(context)) {
    if (value && typeof value === "object") {
      const nested = (value as Record<string, unknown>).map_url;
          ...
          setState((prev) => ({
            ...
            messages: [
              ...prev.messages,
              {
                ...
                // The agents' structured output arrives in `context`; a map URL
                // is the one part of it the chat can render directly.
                mapUrl: readMapUrl(response.context),
              },
            ],
          }));
          setIsThinking(false);
